...
function Sidebar({ activeTasks, workbenchView, onWorkbenchView, theme, setTheme, onPublish, onSplit, onSetupCensor, onUpdate, onSettings, mobileOpen, setMobileOpen }) {
  const { t } = useI18n();
  function goToWorkbench(view) { onWorkbenchView(view); requestAnimationFrame(() => document.getElementById('workbench')?.scrollIntoView({ behavior: 'smooth' })); setMobileOpen(false); }
  return <aside className={`app-sidebar ${mobileOpen ? 'mobile-open' : ''}`}>
    ...
    <div className="app-sidebar-content">
      <Button variant="primary" icon="plus" className="sidebar-publish" onClick={() => { onPublish(); setMobileOpen(false); }}>{t('nav.createPublish')}</Button>
      <nav className="app-navigation">
        <button className={workbenchView === 'tasks' ? 'active' : ''} onClick={() => goToWorkbench('tasks')}><Icon name="queue"/><span>{t('nav.tasks')}</span>{activeTasks > 0 && <b>{activeTasks}</b>}</button>
        <button className={workbenchView === 'logs' ? 'active' : ''} onClick={() => goToWorkbench('logs')}><Icon name="terminal"/><span>{t('nav.logs')}</span></button>
      </nav>
      ...
      <div className="app-sidebar-bottom"><div className="sidebar-bottom-actions"><IconButton icon={theme === 'dark' ? 'sun' : 'moon'} label={t('nav.toggleTheme')} onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}/><button onClick={() => { onSettings(); setMobileOpen(false); }}><Icon name="settings"/><span>{t('nav.settings')}</span></button></div></div>
    </div>
  </aside>;
}

function QuickPublish({ images, status, scheduler, onPublish, onOpenFolder }) {
  ...
  return <section className="quick-publish">
    ...
    <div className="quick-platforms"><PlatformBadge id="civitai" connected={status.civitai_logged_in}/><PlatformBadge id="pixiv" connected={status.pixiv_logged_in}/></div>
...
function FlowConsoleApp() {
  ...
  const activeTasks = tasks.filter(task => ['running','queued','waiting_input'].includes(task.status)).length;
  return <div className="flow-application">
    <Sidebar activeTasks={activeTasks} workbenchView={workbenchView} onWorkbenchView={setWorkbenchView} theme={theme} setTheme={setTheme} onPublish={() => setDialog('publish')} onSplit={() => setDialog('split')} onSetupCensor={() => runTask(4).catch(error => notify(localizedError(error, t), 'error'))} onUpdate={() => runTask(5).catch(error => notify(localizedError(error, t), 'error'))} onSettings={() => setDialog('settings')} mobileOpen={mobileOpen} setMobileOpen={setMobileOpen}/>
    {mobileOpen && <button className="mobile-menu-scrim" aria-label={t('nav.closeMenu')} onClick={() => setMobileOpen(false)}/>}
    <main className="app-main">
      <header className="app-topbar"><div><h1>{t('app.workspace')}</h1><span className={connected ? 'connected' : ''}><i/>{connected ? t('app.connected') : t('app.reconnecting')}</span></div><div><span className="app-version">v{status.version || t('common.notAvailable')}</span><IconButton icon="settings" label={t('nav.settings')} onClick={() => setDialog('settings')}/></div></header>
      <div className="app-content">
        <QuickPublish images={images} status={status} scheduler={scheduler} onPublish={() => setDialog('publish')} onOpenFolder={openFolder}/>
