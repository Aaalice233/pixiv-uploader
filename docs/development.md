...
pixiv_uploader/                 Python 应用包
  ...
  humanize.py                   浏览器自动化拟人化原语（鼠标轨迹、动态打字、自适应节奏）
...
`pixiv_uploader/humanize.py` 为 Pixiv 与 Civitai 的可交互浏览器操作注入真人行为特征，目标是在不降低自动化程度的前提下降低服务端人机验证的触发概率。每个浏览器会话创建一个 `HumanSession` 并贯穿该会话使用，Pixiv 与 Civitai 流程各自持有；隐藏文件输入等无法人工点击的浏览器接口仍使用 Playwright 原生能力。

- **会话指纹抽样**：实例创建时随机抽样本次会话的打字基准速度、鼠标速度、停顿倾向等基准值，会话内一致、会话间不同；参数全部自适应，不做用户配置，config.json 与设置页无对应字段。
- **拟人鼠标**（`session.mouse`）：贝塞尔轨迹叠加垂直微抖动与末端过冲修正，步数按距离近似 Fitts 定律，点击前有短暂 dwell；最终通过 Locator actionability 校验确认点击没有被遮挡。`idle_drift()` 只在未检测到可见人机验证时用于页面加载等长等待，`scroll()` 把滚动拆成多段随机距离与停顿。
- **拟人打字**（`session.type_text()`）：逐字符输入，ASCII、假名、汉字和标点分别使用不同的随机间隔，并保留偶发思考停顿；日文标题、中日文混合简介以及中英日标签都使用可靠的 Unicode 文本输入，仅对 ASCII 字母低概率模拟“打错相邻键 → 退格 → 修正”。代码不伪造浏览器自动化无法真实复现的系统 IME composition。普通文本字段校验不符时可清空重打一次；标签输入为增量模式，禁止清空，且每项必须确认已经形成标签后才能输入下一项。
- **自适应节奏（Pacer）**：动作间隔随连续动作数按疲劳系数（上限 1.8×）变缓，约 2% 概率走神停顿。用户配置的投稿间隔是硬性最小值，Pixiv 与 Civitai 都只在此基础上增加 0~60% 随机量，并约有 8% 概率再叠加 2~6 分钟长休息；等待时通过任务活动显示可取消的真实倒计时。点击投稿前固定有 1.5~4 秒的“通读检查”停顿。
- **会话热身**：Pixiv 先用真实投稿页验证 Profile；登录过期时等待登录完成，再浏览首页（滚动 + 停留）并回到已验证的投稿页。相邻投稿之间约 15% 概率插入首页或刚发布作品页的浏览过渡；只访问 Pixiv 官方页面，不做任何对抗验证的行为。

硬约束：交互优先使用拟人轨迹和逐键输入，并在操作后验证真实页面状态；仅在主路径失败时回退 Playwright 原生操作或 JS 赋值。浏览器关闭与取消异常原样透传，关闭后立即停止后续 DOM 操作。模块不改 UA、时区、locale、视口等浏览器指纹，也不引入风险等级、额外冷却或 429 重试机制。
...
- `category`：`workflow` 或 `maintenance`；发布工作台只消费前者，系统维护页只消费后者；
...
上传任务按目标平台和 LLM 开关动态构造阶段，按“初始化 4% + 所有图片加权阶段 94% + 收尾 2%”聚合。多图任务中每张图片占图片处理区间的 `1 / total`，当前图片再按真实阶段权重填充这一份进度；发布工作台必须同时显示当前图片序号、总图片数、已处理数和批次总百分比，不能把单张图片的阶段进度冒充批次进度。失败或取消保留在最后到达的阶段和百分比，不伪装成 100%。长耗时步骤只显示活动动画，不用定时器制造虚假数值。
...
- 侧边栏固定收敛为发布工作台、运行记录和设置，不放“安装模型”“检查更新”等一次性命令，也不与页面内部标签重复；
- 设置是一级页面，通用、Pixiv 处理、LLM 文案与标签、定时发布和系统维护属于其内部分类；新增配置应优先归入现有分类，只有形成独立工作流时才新增侧边栏页面；
- 发布任务使用 `category=workflow` 并进入发布工作台；模型安装、更新等环境操作使用 `category=maintenance`，只在系统维护页展示其状态和输出；
