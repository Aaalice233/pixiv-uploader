...
`pixiv_uploader/humanize.py` 为 Pixiv 与 Civitai 的全部浏览器自动化注入真人行为特征，目标是在不降低自动化程度的前提下降低服务端人机验证的触发概率。模块对外只暴露 `HumanSession`：每个浏览器会话创建一个实例并贯穿该会话使用，Pixiv 与 Civitai 流程各自持有。

- **会话指纹抽样**：实例创建时随机抽样本次会话的打字基准速度、鼠标速度、停顿倾向等基准值，会话内一致、会话间不同；参数全部自适应，不做用户配置，config.json 与设置页无对应字段。
- **拟人鼠标**（`session.mouse`）：贝塞尔轨迹叠加垂直微抖动与末端过冲修正，步数按距离近似 Fitts 定律，点击前有短暂 dwell；`idle_drift()` 在验证码/页面加载等长等待期间低概率漂移鼠标，`scroll()` 把滚动拆成多段随机距离与停顿。
- **拟人打字**（`session.type_text()`）：逐字符输入，间隔区分词内爆发、标点停顿与偶发思考停顿；仅对 ASCII 字母以低概率模拟“打错相邻键 → 退格 → 修正”，CJK 经 IME 输入不模拟。输入后校验 `input_value()`，不符自动清空重打一次，仍不符抛 `HumanTypingError`。
- **自适应节奏（Pacer）**：动作间隔随连续动作数按疲劳系数（上限 1.8×）变缓，约 2% 概率走神停顿，每篇投稿完成后约 8% 概率叠加 2~6 分钟长休息，点击投稿前固定有 1.5~4 秒的“通读检查”停顿。所有停顿经 `sleep_with_cancel` 可取消。
- **会话热身**：Pixiv 批次首次投稿前先浏览首页（滚动 + 停留），相邻投稿之间约 15% 概率插入首页或刚发布作品页的浏览过渡；只访问 Pixiv 官方页面，不做任何对抗验证的行为。

硬约束：拟人化层任何异常必须允许调用方回退到原有的直接操作（fill / JS 赋值 / force 点击），投稿功能正确性绝不受拟人化影响；浏览器关闭与取消异常原样透传不吞。模块不改 UA、时区、locale、视口等浏览器指纹（指纹突变本身会触发风控），也不引入风险等级、冷却或 429 重试机制。
